...
xEvalCoefficient::xEvalCoefficient(double omega_, double beta_, double sigmaF_, double d_) : omega(omega_), beta(beta_), sigmaF(sigmaF_), d(d_)
{
    ...
    const double phi = 0.98, sigma = 3.75e3, alpha = 1.17, lambda_prime = 742e-6, lambda = 110e-6;
    ...
    ////// solid part////////
    const double rho_1 = 22.1, nu_p = 0.39, E = 70e3, eta = 0.265;
    ...
    M1 << k_ax/(rho_a*pow(omega,2)), k_1x/(K_eq_til*pow(delta_1,2)), k_2x/(K_eq_til*pow(delta_2,2)), ky*mu_3,
        -1.+sigma*d*k_ax/(omega*rho_a), -1. ,-1., 0.,
        0., 2.*N*pow(k_1x,2)/(mu_1*K_eq_til*pow(delta_1,2))+A_hat/(mu_1*K_eq_til), 2.*N*pow(k_2x,2)/(mu_2*K_eq_til*pow(delta_2,2))+A_hat/(mu_2*K_eq_til), 2.*N*ky*k_3x,
        0., -2.*N*ky*k_1x/(mu_1*K_eq_til*pow(delta_1,2)), -2.*N*ky*k_2x/(mu_2*K_eq_til*pow(delta_2,2)), N*pow(k_3x,2)-N*pow(ky,2);
    M2 << k_ax/(rho_a*pow(omega,2)), k_ax/(rho_a*pow(omega,2)), k_2x/(K_eq_til*pow(delta_2,2)), ky*mu_3,
        1.+sigmaF*d*k_ax/(omega*rho_a), -1.+sigma*d*k_ax/(omega*rho_a) ,-1., 0.,
        0.,  0., 2.*N*pow(k_2x,2)/(mu_2*K_eq_til*pow(delta_2,2))+A_hat/(mu_2*K_eq_til), 2.*N*ky*k_3x,
        0., 0., -2.*N*ky*k_2x/(mu_2*K_eq_til*pow(delta_2,2)), N*pow(k_3x,2)-N*pow(ky,2);
    M3 << k_ax/(rho_a*pow(omega,2)), k_1x/(K_eq_til*pow(delta_1,2)), k_ax/(rho_a*pow(omega,2)), ky*mu_3,
        1.+sigmaF*d*k_ax/(omega*rho_a), -1. ,-1.+sigma*d*k_ax/(omega*rho_a), 0.,
        0., 2.*N*pow(k_1x,2)/(mu_1*K_eq_til*pow(delta_1,2))+A_hat/(mu_1*K_eq_til), 0., 2.*N*ky*k_3x,
        0., -2.*N*ky*k_1x/(mu_1*K_eq_til*pow(delta_1,2)), 0., N*pow(k_3x,2)-N*pow(ky,2);
    M4 << k_ax/(rho_a*pow(omega,2)), k_1x/(K_eq_til*pow(delta_1,2)), k_2x/(K_eq_til*pow(delta_2,2)), k_ax/(rho_a*pow(omega,2)),
        1.+sigmaF*d*k_ax/(omega*rho_a), -1. ,-1., -1.+sigma*d*k_ax/(omega*rho_a),
        0., 2.*N*pow(k_1x,2)/(mu_1*K_eq_til*pow(delta_1,2))+A_hat/(mu_1*K_eq_til), 2.*N*pow(k_2x,2)/(mu_2*K_eq_til*pow(delta_2,2))+A_hat/(mu_2*K_eq_til), 0.,
        0., -2.*N*ky*k_1x/(mu_1*K_eq_til*pow(delta_1,2)), -2.*N*ky*k_2x/(mu_2*K_eq_til*pow(delta_2,2)), 0.; 
    b << k_ax/(rho_a*pow(omega,2)), -1., 0., 0.;
    ...
    // cout<<"solver eigen"<<endl;

    // cout<<"Transmission 2: "<<std::setprecision(20)<<T2<<endl;
    // cout<<"Transmission 2: "<<std::setprecision(20)<<T3<<endl;

    // valtype det_M = M.determinant();
    // valtype det_M1 = M1.determinant();
    // valtype det_M2 = M2.determinant();
    // valtype det_M3 = M3.determinant();
    // valtype det_M4 = M4.determinant();
    // R = det_M1/det_M;
    // T1 = det_M2/det_M;
    // T2 = det_M3/det_M;
    // T3 = det_M4/det_M;
    // cout<<"Cramer rule"<<endl;
    // cout<<"equivalent fluid density : "<<std::setprecision(20)<<rho_eq_til<<endl;
    // cout<<"gamma_til : "<<std::setprecision(20)<<gamma_til<<endl;
    // cout<<"wave number3 : "<<std::setprecision(20)<<k_2x<<endl;
    // cout<<"resist: "<<sigmaF<<endl;
 
    // beta = 45, sigmaF = 775e3, d=0.45e-3  f=12560
    R = (0.2541327602225452-0.023005859699729063*j);
    T1 = ((0.0008893344353146778-0.0019409116807776116*j));
    T2 = ((0.8098308293967889-0.03474176265515239*j));
    T3 = ((1.2771842892283976e-12-4.721210247167941e-13*j));
    ...
    // T3 = (1.02413173010619e-12-3.528170974331122e-13*j);
        ...
        // beta = 30, sigmaF = 775e3, d=0.45e-11  f=12560
    // R = (0.10328102400547859-0.05494084221046457*j);
    // T1 = ((0.0012878208971695883-0.002731751535276035*j));
    // T2 = (1.1019931965792817-0.05220909107521382*j);
    // T3 = ((1.3655177151735422e-12-4.4751079029908017e-13*j));
...
void xEvalExactBiotUs::operator()(const xGeomElem*  geo_appro, const xGeomElem* geo_integ, xtensor::xVector<std::complex<double> >& res) const{
    ...
    {
        res(0) = 100.*(-j*k_1x*A_1*exp(-j*k_1x*x-j*ky*y)-j*k_2x*A_2*exp(-j*k_2x*x-j*ky*y)-j*ky*T3*exp(-j*k_3x*x-j*ky*y));
        res(1) = 100.*(-j*ky*A_1*exp(-j*k_1x*x-j*ky*y)-j*ky*A_2*exp(-j*k_2x*x-j*ky*y) +j*k_3x*T3*exp(-j*k_3x*x-j*ky*y));
    }
...
void xEvalExactBiotTraction::operator()(const xGeomElem*  geo_appro, const xGeomElem* geo_integ, xtensor::xTensor2<std::complex<double> >& res) const{
    ...
    valtype e_xx = 100.*(-A_1*pow(k_1x,2)*exp(-j*k_1x*x-j*ky*y)-A_2*pow(k_2x,2)*exp(-j*k_2x*x-j*ky*y)-T3*k_3x*ky*exp(-j*k_3x*x-j*ky*y));
    valtype e_yy = 100.*(-A_1*pow(ky,2)*exp(-j*k_1x*x-j*ky*y)-A_2*pow(ky,2)*exp(-j*k_2x*x-j*ky*y)+T3*k_3x*ky*exp(-j*k_3x*x-j*ky*y));
    valtype e_xy = 100.*(0.5*(-2.*k_1x*ky*A_1*exp(-j*k_1x*x-j*ky*y)-2.*k_2x*ky*A_2*exp(-j*k_2x*x-j*ky*y)+(pow(k_3x,2)-pow(ky,2))*T3*exp(-j*k_3x*x-j*ky*y)));
...
// 2D computation numerically   
    ...
    {
        res(0) = 100.*(-A_1*j*mu_1*k_1x*exp(-j*k_1x*x-j*ky*y) -A_2*j*mu_2*k_2x*exp(-j*k_2x*x-j*ky*y) -T3*j*mu_3*ky*exp(-j*k_3x*x-j*ky*y));
        res(1) = 100.*(-A_1*j*mu_1*ky*exp(-j*k_1x*x-j*ky*y) -A_2*j*mu_2*ky*exp(-j*k_2x*x-j*ky*y) +T3*j*mu_3*k_3x*exp(-j*k_3x*x-j*ky*y));
    }
...
void xEvalExactBiotPressure::operator()(const xGeomElem*  geo_appro, const xGeomElem* geo_integ, std::complex<double>& res) const{
    ...
    if (cdgx<0.) {
        res = 100.*(exp(-j*k_ax*x-j*ky*y)+R*exp(j*k_ax*x-j*ky*y));
    }
    else
    {
        res = 100.*(T1*exp(-j*k_1x*x-j*ky*y) + T2*exp(-j*k_2x*x-j*ky*y));
    }
...
void xEvalExactBiotVelocity::operator()(const xGeomElem*  geo_appro, const xGeomElem* geo_integ, xtensor::xVector<std::complex<double> >& res) const{
   ...
   const double rho_a = 1.213;
    ...
    if (cdgx<0.) {
        ...
        res(0) = 100.*(-j*k_ax*exp(-j*k_ax*x-j*ky*y)+j*k_ax*R*exp(j*k_ax*x-j*ky*y));
        res(1) = 100.*(-j*ky*exp(-j*k_ax*x-j*ky*y)-j*ky*R*exp(j*k_ax*x-j*ky*y));
    }
    else
    {
        res(0) = 0;
