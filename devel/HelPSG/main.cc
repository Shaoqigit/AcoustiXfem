...
//define interface
    ...
    // xLevelSet ls(all, xCompl(xCylinder(xtensor::xPoint(0.,0.,0.), xtensor::xVector<>(0., 0., 1.), 0.12)));

    helmholtzNdPSG helmholtzPSG(data, MPI_COMM_WORLD, parsedinfos);
    helmholtzPSG.setLevelSet(ls);
    helmholtzPSG.TreatmentOfFormulation(parsedinfos, m2);
